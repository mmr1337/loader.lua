...
task.spawn(function()
    ...
    -- [FIX 1] everAlive: позиции где башня реально существовала на карте
    -- Rebuild НЕ трогает позиции которые макрос ещё не поставил
    local everAlive = {}
        ...
        task.spawn(function()
            ...
            while true do
                if #jobQueue > 0 then
                    ...
                    -- [FIX 3] Перед ребилдом проверяем: не стала ли позиция sold
                    -- пока задание ждало в очереди
                    ...
                    elseif not ShouldSkipTower(job.x, job.towerName, job.firstPlaceLine) then
    ...
    while true do
        ...
        if macroContent and #macroContent > 10 then
            ...
            if macroHash ~= lastMacroHash then
                ...
                if ok and type(macro) == "table" then
                    ...
                    -- [FIX 2] Парсинг с пониманием sell→replace цепочек
                    -- Sell очищает старые записи для X
                    -- Новый Place после Sell на том же X отменяет sold
                    for i, entry in ipairs(macro) do
                        ...
                        elseif entry.SellTower then 
                            ...
                            if x then
                                soldAxis[x] = true
                                towersByAxis[x] = nil  -- старые записи больше не актуальны
                                everAlive[x] = nil      -- сброс: новая башня ещё не жила
                            end
                        elseif entry.TowerPlaced and entry.TowerVector then 
                            local x = tonumber(entry.TowerVector:match("^([%d%-%.]+),"))
                            if x then
                                soldAxis[x] = nil  -- новая башня отменяет sold
                                towersByAxis[x] = {}
        ...
        for hash, tower in pairs(TowerClass.GetTowers()) do
            if tower.SpawnCFrame and typeof(tower.SpawnCFrame) == "CFrame" then
                ...
                existingTowersCache[x] = true
                -- [FIX 1] Отмечаем: эта позиция была жива
                everAlive[x] = true
            end
        end
        ...
        for x, records in pairs(towersByAxis) do
            ...
            elseif not existingTowersCache[x] then
                -- [FIX 1] Только если башня БЫЛА жива и умерла
                -- Если everAlive[x] == nil — макрос ещё не поставил, не трогаем
                if not everAlive[x] then
                    -- макрос ещё не дошёл до этой башни — пропускаем
                elseif not activeJobs[x] then
                    recordTowerDeath(x)
