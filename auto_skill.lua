...
local function getStaticBehavior(entityType)
  ...
  local v = SEBCache[k]; if v then return v end
...
local function getBuffType(buffName)
  ...
  local v = BuffTypeCache[k]; if v then return v end
...
local function cfgIsAttackBuff(cfg)
  local v = AttackBuffCfgCache[cfg]; if v~=nil then return v end
  v = isAttackBuffList(cfg.TowerBuffNames)
  AttackBuffCfgCache[cfg] = v
  return v
end

local MOBSTER_DELAY = 0.25
...
local TStunCache, TKritzCache = {}, {}
local TRangeCache, TRangeSqCache = {}, {}
local TDPSCache  = {}
local ALCache, ACfgCache = {}, {}
local TStealthCache = {}
local TPath2Cache   = {}
local TPosCache     = {}

local AbilityCooldownUntil = {}
...
local function getTPos(tower)
  local h = tower.Hash
  local v = TPosCache[h]; if v then return v end
...
local function getRange(tower)
  local h = tower.Hash
  local v = TRangeCache[h]; if v then return v end
...
local function getRangeSq(tower)
  local h = tower.Hash
  local v = TRangeSqCache[h]; if v then return v end
  local r = tower:GetCurrentRange()
  TRangeCache[h] = r; v = r * r
  TRangeSqCache[h] = v
  return v
end
...
local function refreshBuffState(hash, tower)
  local bh = tower.BuffHandler; if not bh then return end
...
local function getDPS(tower)
  local h = tower.Hash
  local v = TDPSCache[h]; if v then return v end
  refreshDPS(h, tower)
...
local function buildAList(hash, ah)
  ...
  for i = 1, 3 do
    local n  = map[i]; if not n then break end
    local ab = abs[n]
    if ab and not ab.Tower then ab=nil end
    if ab and ab.Config and (ab.Config.Passive or ab.Config.CustomTriggered) then ab=nil end
...
local function onTAdd(hash, tower)
  ...
  local r = tower:GetCurrentRange()
  TRangeCache[hash]=r; TRangeSqCache[hash]=r*r
...
local function onTRemove(hash)
  ...
  if al then
    for i=1,3 do
      local ab=al[i]
      if ab then AbilityCooldownUntil[ab]=nil end
    end
  end
...
local function hookAHC()
  ...
  if not ahc and filtergc then
    local c = filtergc("table",{keys=true})
    if c then for _, v in _ipairs(c) do
  ...
  hookFn(ahc, "SetTower", function(orig, self, tower, ...)
    ...
    if tower and tower.Hash then
      ...
      local oldAL = ALCache[tower.Hash]
      if oldAL then
        for i=1,3 do
          local ab=oldAL[i]
          if ab then AbilityCooldownUntil[ab]=nil end
      ...
      local newAL = ALCache[tower.Hash]
      if newAL then
        for i=1,3 do
          local ab=newAL[i]
          if ab then AbilityCooldownUntil[ab]=nil end
  ...
  local ac
  if ahcPath2 then local ok,m=pcall(require,ahcPath2); if ok then ac=m end end
  if not ac then
    local ok, upvals = pcall(debug.getupvalues, ahc._GenerateAbilities)
    if ok and upvals then
      for _, v in _pairs(upvals) do
        if _type(v)=="table" and _rawget(v,"BeginCooldown") and _rawget(v,"CanUse") then
          ac=v; break
        end
      end
    end
  ...
  if not ac and filtergc then
    local c = filtergc("table",{keys=true})
    if c then for _, v in _ipairs(c) do
      if _rawget(v,"BeginCooldown") and _rawget(v,"CanUse") and _rawget(v,"Use") then
        ac=v; break
      end
    end end
  end
  ...
  if ac then
    ...
    hookFn(ac, "BeginCooldown", function(orig, ab, ...)
      ...
      AbilityCooldownUntil[ab] = _tick() + (ab.CooldownRemaining or 0)
    end)
  end
...
local function updateBuffCache(tower)
  local hash = tower and tower.Hash; if not hash then return end
  TRangeCache[hash]=nil; TRangeSqCache[hash]=nil; TDPSCache[hash]=nil
  refreshBuffState(hash, tower); refreshDPS(hash, tower)
end
...
local function hookTC()
  ...
  hookFn(TowerClass, "AbilityUsedOnServer", function(orig, tower, data, ...)
    local ab = orig(tower, data, ...)
    if ab then


      local remaining = ab.CooldownRemaining or 0
      if remaining > 0 then
        AbilityCooldownUntil[ab] = _tick() + remaining
      else
        AbilityCooldownUntil[ab] = nil
      end
    end
    return ab
  end)
  ...
  hookFn(TowerClass, "SetAlive", function(orig, tower, data, ...)
    orig(tower, data, ...)
    local hash = tower and tower.Hash; if not hash then return end
    local al = ALCache[hash]
    if al then
...
local function getEnemyAttackRange(e)
  ...
  if ah then local r=ah.AttackRange or ah.Range; if r then return r end end
    ...
    if _type(abs)=="table" then
      for _, ab in _pairs(abs) do
        if ab and ab.Config then
          local r = ab.Config.AttackRange or ab.Config.ManualAimCustomRange or ab.Config.EffectRadius
...
local function buildESnap(enemies)
  ...
  for _, e in _ipairs(enemies) do
    local ep = e:GetPosition(); if not ep then continue end
    local hash = e.Hash
    ...
    buf_writef32(ES_buf, base+28, ep.Y)
...
local function getFarEnemy(pos, range, noAir)
  ...
  if best then
    ...
    if e and e.IsAlive then
      local rp = e:GetPosition()
      if rp then return rp end
    end
    local b2 = best * ES_STRIDE
    return _V3new(buf_readf32(ES_buf,b2), buf_readf32(ES_buf,b2+28), buf_readf32(ES_buf,b2+4))
  end
end
...
local function getHealDroneTgt(selfHash)
  ...
  for h, t in _pairs(FTowers) do
    ...
    local hh = t.HealthHandler; if not hh then continue end
    local hp, maxHp = hh:GetHealth(), hh:GetMaxHealth()
    if maxHp<=0 or hp>=maxHp then continue end
    local tp = getTPos(t); if not tp then continue end
...
local function markStealthSplash(tPos, now, sr)
  local sr2 = sr*sr
  local tx, tz = tPos.X, tPos.Z
  for h2, t2 in _pairs(FTowers) do
    if not t2.IsAlive then continue end
    local tp2 = getTPos(t2); if not tp2 then continue end
    local dx=tp2.X-tx; local dz=tp2.Z-tz
    if dx*dx+dz*dz <= sr2 then StealthTgtLastUsed[h2]=now end
  end
end

local function getStealthTgtPos(hash, now, splashRadius)
  local th = getBestStealthTgt(hash, now); if not th then return nil end
  local tPos = getTPos(FTowers[th]); if not tPos then return nil end
...
local function checkTowers(selfHash, pos, radius, checkHeal, checkStun, selfOnly)
  if selfOnly then
    local t = FTowers[selfHash]; if not t then return false end
    if checkHeal then
...
local function isUsable(ab)
  ...
  if (ab.CooldownRemaining or 0) <= 0 then
    AbilityCooldownUntil[ab] = nil
  end


  local until_t = AbilityCooldownUntil[ab]
  if until_t and _tick() < until_t then return false end


  if ab.Config and ab.Config.HotbarData then
    return (ab.CooldownRemaining or 0) <= 0
  else
    return ab:CanUse()
  end
end
...
local function hasUsable(al)
  ...
  for i=1,3 do
    local ab=al[i]; if ab==nil then break end
...
local function useAb(ab)
  ...
  if not ab:Use() then AbilityCooldownUntil[ab] = nil end
end
...
local function cachePaths()
  if _next(PathEnds) then return end
...
local function getMedicTgt(medicTower, medicHash)
  local medicPos = getTPos(medicTower); if not medicPos then return nil end
  ...
  for hash, tower in _pairs(FTowers) do
    ...
    local tp=getTPos(tower); if not tp then continue end
...
local function getMobTgt(tower, now)
  local pos=getTPos(tower); if not pos then return nil end
...
local function procMobster(tower, hash, now)
  local al=ALCache[hash]; if not hasUsable(al) then return end
...
local function procAttack(attackHash, now)
  local atTower=FTowers[attackHash]; if not atTower then return end
  local atPos=getTPos(atTower); if not atPos then return end
  ...
  for hash, tower in _pairs(FTowers) do
    ...
    local al=ALCache[hash]; if not al then continue end
    ...
    if cfgs then
      for i=1,3 do
        ...
        and cfgIsAttackBuff(cfg) and isUsable(ab) then
          AttackTriggered[hash]=true; break
        end
      end
    end
    ...
    if tower.Type=="Medic" then
      local p2=TPath2Cache[hash] or 0; if p2<4 then continue end
      ...
      for i=1,3 do
        local ab=al[i]; if not ab then break end
        if isUsable(ab) then
...
local function runMain(now)
  ...
  for hash, tower in _pairs(FTowers) do
    ...
    for idx=1,3 do
      ...
      local cqd      = cfg.CustomQueryData
      local aimRange = (cqd and cqd.Range) or range
      local cr    = cfg.RadiusDamage and cfg.EffectRadius or aimRange
      local tPos  = nil
      local allow = true
      local needPos = cfg.IsManualAimAtGround or cfg.IsManualAimAtPath or cfg.ManualAimInfiniteRange


      if cfg.TargetSelf and not needPos then
        local isBuff   = cfg.TowerBuffNames~=nil and not cfg.HealPercentage and not cfg.HealAmount and not cfg.Unstun
        local isHeal   = cfg.HealPercentage~=nil or cfg.HealAmount~=nil
        local isUnstun = cfg.Unstun==true
        local canUse   = false
        if isBuff then

          canUse = atkFired==true
          if canUse then AttackTriggered[hash]=nil; atkFired=nil end
        elseif isHeal or isUnstun then

          canUse = checkTowers(hash, nil, nil, isHeal, isUnstun, true)
        end
        if canUse then enqueue(hash, idx, nil, nil) end
        continue
      end
      ...
      if needPos then
        ...
        if useStealthTgt then
          ...
          elseif spawnsPath then
            tPos = cfg.ManualAimInfiniteRange and getRelicTgt()
                or getFarEnemy(pos, cfg.ManualAimCustomRange or cr, noAir)
          elseif cfg.ManualAimInfiniteRange then
...
RunService.Heartbeat:Connect(newcclosure(function()
  ...
  if doSnap then
    ...
    if AtkQueueN > 0 then
      local t2 = AtkQueueTime
      for i=1,AtkQueueN do procAttack(AtkQueue[i], t2); AtkQueue[i]=nil end
      AtkQueueN = 0
    end
  end
...
task.spawn(function()
  ...
  while true do
    if SQ_sz > 0 then
      local h = SQ_hd
      local ok, serverCooldown = TowerUseAbilityRequest:InvokeServer(
      ...
      if ab then
        if ok and serverCooldown then
          ...
          AbilityCooldownUntil[ab] = _tick() + serverCooldown
        elseif not ok then
